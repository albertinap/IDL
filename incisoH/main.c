...
int main()
{
    ...
    while (res == 1)
        res = validarX(x, b, m);
    /*en esta instancia todos los valores son los validos*/
    /*--------------------------------------------------------------*/
    int xb[16]; // binario
    ...
    int ordenada = convertirAEnteroPF(m) * convertirAEnteroPF(x) + convertirAEnteroPF(b);
...
int dentroRangoX(char *x, char *b, char *m)
{
    ...
    if (mpf * xpf + bpf > max || mpf * xpf + bpf < min)
        return 2;
